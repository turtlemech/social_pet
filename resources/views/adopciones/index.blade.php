...
<div class="min-h-screen bg-[#f3f4f6] py-8 px-4 sm:px-6">
    ...
    <a href="{{ route('adopciones.create') }}"
       class="bg-white text-[#0d9488] px-6 py-3 rounded-xl font-bold shadow hover:shadow-lg hover:scale-105 transition flex items-center gap-2">
    ...
    <a href="{{ route('adopciones.solicitudes') }}"
       class="bg-white text-[#0d9488] px-6 py-3 rounded-xl font-bold shadow hover:shadow-lg hover:scale-105 transition">
                ...
                <div class="adoption-card bg-white rounded-2xl overflow-hidden shadow-sm flex flex-col h-full">
                    ...
                    <div class="relative h-56 overflow-hidden group">
                        ...
                        <div class="absolute top-3 left-3">
                            ...
                            @if($ado->est_ado == 'pendiente')

    🟢 Disponible

@elseif($ado->est_ado == 'aprobada')
...
@endif
                    ...
                    <div class="p-5 flex flex-col flex-grow">
                        ...
                        {{-- BOTÓN --}}
                        @if($ado->us_act == auth()->id())

    <div class="flex gap-2 mt-auto">

        <form action="{{ route('adopciones.adoptada', $ado->id) }}" method="POST" class="flex-1">
            @csrf
            @method('PATCH')

            <button
    type="submit"
    onclick="return confirm('¿Confirmas que la mascota ya fue adoptada?')"
    class="w-full py-3 rounded-xl bg-green-600 hover:bg-green-700 text-white font-bold">
    ✅ Marcar adoptada
</button>
        </form>

        <form action="{{ route('adopciones.cancelar', $ado->id) }}" method="POST" class="flex-1">
            @csrf
            @method('PATCH')

            <button
    type="submit"
    onclick="return confirm('¿Seguro que deseas cancelar esta adopción?')"
    class="w-full py-3 rounded-xl bg-red-600 hover:bg-red-700 text-white font-bold">
    ❌ Cancelar
</button>
        </form>
...
@else
    ...
    <a
    href="{{ route('adopciones.solicitar', $ado->id) }}"
    class="btn-sp w-full py-3 rounded-xl text-white font-bold flex items-center justify-center">
